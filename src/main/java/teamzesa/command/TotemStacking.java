...
public class TotemStacking implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (validMinimumTotemCount(itemList)) {
            ComponentExchanger.playerAnnouncer(player,"2개 이상의 토템을 가지고 있으셔야 합니다.", TextColor.color(0xF80040));
            return false;
        }

        if (validTotemCommand(itemList)) {
            ComponentExchanger.playerAnnouncer(player,"합칠 토템이 없습니다.", TextColor.color(0xF80040));
            return false;
        }
...
//        p.sendMessage("총 토템 " + String.valueOf(totalAmount));
        ...
        ComponentExchanger.playerAnnouncer(player,"합칠 토템이 없습니다.", TextColor.color(255,255,0));
        return true;
    }
